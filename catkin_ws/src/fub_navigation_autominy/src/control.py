...
class VectorfieldController:
    def __init__(self):
        ...
        self.file_path = rospack.get_path('fub_navigation_autominy') + '/src/'
        #self.lut = np.load(self.file_path + 'LUT.npy')
        self.matrix_lane_1 = np.load(self.file_path + 'matrix50cm_lane1.npy')
        self.matrix_lane_2 = np.load(self.file_path + 'matrix-50cm_lane2.npy')
    ...
    def callback(self, data):
        ...
        sat_kp = 1000
        steering = min(sat_kp,max(-sat_kp,self.Kp * angle)) + self.Kd * ((angle - self.last_angle) / dt) + self.Ki * self.integral_error
        ...
        if not self.shutdown_:
            msg = SpeedCommand()
            #msg.value = speed*(1+0.75*np.exp(-5*angle))
            #speed.value=2-1.75*min(abs(steer.value),1.0)
            if self.cnt_init < 50:
                msg.value = speed
                self.cnt_init += 1
                print(self.cnt_init)
            else:
                msg.value = 0.5-0.2*np.tanh(2.0*abs(steering))
            print(msg.value)
            msg.header.frame_id = "base_link"
            msg.header.stamp = rospy.Time.now()
            #print("Hello from pub_speed")
    ...
    def lane_change(self):
        if (rospy.Time.now() - self.last_lane_change).to_sec() < 2.0:
            return
    ...
    def on_lidar(self, msg):
        ...
        if len(points) > 0:
            ...
            for (x, y) in points:
                ...
                #if self.lut[xi,yi]:
                if 0 <= xi < 600 and 0 <= yi < 600:
                #if 0 <= xi < 239 and 0 <= yi < 179:
                    if self.lane == 1:
                    ...
                    if dist < 0.2:
                        points_on_track += 1
        if points_on_track > 5:    
            #pass
            print(points_on_track)
            self.lane_change()
